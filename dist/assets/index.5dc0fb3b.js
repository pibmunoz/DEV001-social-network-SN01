...
const saveDataFromUsers = async (name2, country, usersUid, email, password) => {
  const docs = sa(db, "users", usersUid);
  ml(docs, {
    name: name2,
    countries: country,
    user: usersUid,
...
const submitRegister = (email, password) => createUserWithEmailAndPassword(auth, email, password);
const googleLogIn = () => {
  const provider = new GoogleAuthProvider();
  return signInWithPopup(auth, provider);
};
const forgotPassword = (email) => sendPasswordResetEmail(auth, email);
const logInHome = (email, password) => signInWithEmailAndPassword(auth, email, password);
const updateInfo = (currentUser, name2) => updateProfile(currentUser, {
  displayName: name2
});
const updatePhoto = (currentUser, photo) => updateProfile(currentUser, {
  photoUrl: photo
});
const savePost = (textOfEachPost, nameOfUser, usersId, creationDate, likes) => {
  pl(ea(db, "posts"), {
    textOfEachPost,
    nameOfUser,
    usersId,
...
const getSavePosts = (id2) => al(sa(db, "posts", id2));
const getPost = (callback) => {
  Il(ea(db, "posts"), callback);
};
const getUser = (userId) => {
  const docRef = sa(db, "users", userId);
  return al(docRef);
};
const functionDeleteEachPost = (id2) => yl(sa(db, "posts", id2));
const updatePost = (id2, newPost) => gl(sa(db, "posts", id2), newPost);
const updateLikes = (id2, likes) => gl(sa(db, "posts", id2), likes);
const signOutUser = () => signOut(auth);
const storage = () => getStorage();
const downloadUrl = (uploadTask) => getDownloadURL(uploadTask);
const refFunction = (storageFunction, newPhoto) => ref(storageFunction, newPhoto);
const uploadToStorage = (reference, data, metadata) => uploadBytesResumable(reference, data, metadata);
const updateUsers = (id2, petName, type, description) => gl(sa(db, "users", id2), petName, type, description);
const viewForHome = () => {
...
`;
  ...
  homeDiv.querySelector("#buttonSignIn").addEventListener("click", () => {
    ...
    logInHome(email, password).then((userCredential) => {
      ...
      error.code === AUTH_ERROR_CODES_MAP_DO_NOT_USE_INTERNALLY.INVALID_PASSWORD || error.code === AUTH_ERROR_CODES_MAP_DO_NOT_USE_INTERNALLY.USER_DELETED ? textForAlert.innerHTML = "Wrong password" : false;
    });
  });
  return homeDiv;
};
const viewForRegister = () => {
  ...
  registerDiv.querySelector("#signUp").addEventListener("click", (e) => {
    ...
    const name2 = registerDiv.querySelector("#fName").value;
    ...
    if (email === "" || password === "" || name2 === "" || country === "") {
      textMessageSecret.innerHTML = "Fill the empty inputs!!!";
      return textMessageSecret;
    }
    submitRegister(email, password).then((userCredential) => {
      const usersUid = userCredential.user.uid;
      saveDataFromUsers(name2, country, usersUid, email, password);
      const currentUser = userCredential.user;
      updateInfo(currentUser, name2);
      ...
      error.code;
      error.message;
      if (error.code === AUTH_ERROR_CODES_MAP_DO_NOT_USE_INTERNALLY.EMAIL_EXISTS) {
...
const viewForProfile = () => {
  ...
  photoForProfile.addEventListener("change", (event) => {
    ...
    const usersRef = sa(db, "users", user.uid);
    ...
    downloadUrl(uploadTask.snapshot.ref).then((downloadURL) => {
      gl(usersRef, {
...
`;
  ...
  atras.addEventListener("click", () => {
    postDiv.querySelector(".carousel");
    carousel();
  });
  ...
  buttonShowPost.addEventListener("click", async () => {
    getPost((querySnapshot) => {
      ...
      const dataSort = arrayForPost.sort(
        (a, b2) => new Date(b2[0].creationDate) - new Date(a[0].creationDate)
      );
